...
# 3 vector Jet(1, 2, 3)
mssmhbb.Branch('pt123', addressof(tree_out, 'pt123'), 'pt123/F')
mssmhbb.Branch('eta123', addressof(tree_out, 'eta123'), 'eta123/F')
mssmhbb.Branch('phi123', addressof(tree_out, 'phi123'), 'phi123/F')
mssmhbb.Branch('m123', addressof(tree_out, 'm123'), 'm123/F')
# ScalarHT
mssmhbb.Branch('ht', addressof(tree_out, 'ht'), 'ht/F')

NumEvent, NumEvent_passCont1, NumEvent_passCont2 = 0, 0, 0
...
#for entry in range(10):
    ...
    NumEvent += 1
    print("all events: ", NumEvent)  
    #print("branchJet.GetEntries(): ", branchJet.GetEntries())
    #print("nbjets_: ", nbjets_)

    if branchJet.GetEntries() == 3 and nbjets_ >= 2:

        NumEvent_passCont1 += 1
        #print("number of events in conditions: ", NumEvent_passCont1, NumEvent, branchJet.GetEntries(), nbjets_)
        ...
        dijet13 = pjet1 + pjet3
        dijet23 = pjet2 + pjet3
        jet123 = pjet1 + pjet2 + pjet3
        
        if ((jet1.PT >= 30) and
            (jet2.PT >= 30) and
            (jet3.PT >= 30) and
            (abs(jet1.Eta) < 2.4) and 
            (abs(jet2.Eta) < 2.4) and
            (abs(jet3.Eta) < 2.4)):

            NumEvent_passCont2 += 1
            #print("number of events in conditions: ", NumEvent_passCont2, NumEvent_passCont1, NumEvent, branchJet.GetEntries(), nbjets_)
            print(f"events in conditions: {NumEvent_passCont2} at {NumEvent}")

            tree_out.njets = int(branchJet.GetEntries())
            tree_out.nbjets = nbjets_
            ...
            tree_out.pt123  = float(jet123.Pt())
            tree_out.eta123 = float(jet123.Eta())
            tree_out.phi123 = float(jet123.Phi())
            tree_out.m123   = float(abs(jet123.M()))
    ...
    if branchJet.GetEntries() >= 4 and nbjets_ >= 2:
        ...
        if ((jet1.PT >= 30) and
            ...
            tree_out.m1234   = float(abs(jet1234.M()))
